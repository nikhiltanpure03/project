...
function UserComponent() {
  ...
  useEffect(() => {
    ...
    getAccountBookings(account.id)
      .then((response) => setBookingCount(response.data.length))
      .catch(() => {});
  }, [account?.id]);
      ...
      <main className="user-page">
        ...
        <section className="user-details-panel">
          <div className="panel-heading">
            <div>
              <p className="eyebrow dark">Profile information</p>
              <h2>Personal details</h2>
            </div>
            <span className="status-pill positive">Stored account</span>
          </div>
          ...
          <div className="user-actions">
            <Link to="/admin/bookings" className="secondary-btn">
              Check my bookings ({bookingCount})
            </Link>
            <Link to="/discover" className="text-btn">
              Back to stays <span aria-hidden="true">↗</span>
            </Link>
          </div>
        </section>
      </main>
