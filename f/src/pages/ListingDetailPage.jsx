...
function ListingDetailPage() {
  ...
  const formattedCheckIn = booking.checkIn ? new Date(`${booking.checkIn}T00:00:00`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Not selected'
  const formattedCheckOut = booking.checkOut ? new Date(`${booking.checkOut}T00:00:00`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Not selected'
  const maskedCard = booking.card ? `•••• ${booking.card.replace(/\D/g, '').slice(-4)}` : 'Not provided'

  if (paid) {
    return (
      <div className="page-shell detail-shell">
        <Link to="/discover" className="back-link">Back to stays</Link>
        <div className="booking-success">
          <span className="success-mark">✓</span>
          <p className="eyebrow">Payment accepted</p>
          <h1>Your booking is confirmed.</h1>
          <p>Your {isIndianGuest ? 'Indian guest' : 'international guest'} advance payment of {currency}{advance.toLocaleString()} was accepted for {stay.title}.</p>
          ...
          <div className="booking-details-card">
            ...
            <div className="booking-detail-grid">
              ...
              <div>
                ...
                <strong>{isIndianGuest ? 'Indian guest' : 'International guest'}</strong>
              </div>
              <div>
                <span>Check-in</span>
              ...
              <div>
                <span>Guests</span>
                <strong>{booking.guests} guest{Number(booking.guests) > 1 ? 's' : ''}</strong>
              </div>
              <div>
              ...
              <div>
                <span>Subtotal</span>
                <strong>{currency}{subtotal.toLocaleString()}</strong>
              </div>
              <div>
                <span>Service fee</span>
                <strong>{currency}{serviceFee.toLocaleString()}</strong>
              </div>
              <div>
                <span>Taxes</span>
                <strong>{currency}{taxes.toLocaleString()}</strong>
              </div>
              <div className="booking-total">
                <span>Total paid</span>
                <strong>{currency}{advance.toLocaleString()}</strong>
              </div>
            </div>
          </div>

          <Link to="/discover" className="primary-btn">Explore more stays</Link>
        </div>
      </div>
    )
  }
